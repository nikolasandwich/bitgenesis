...
from scripts.run_v0_joint_zero_charge import initialize
from scripts.verify_v0_joint_zero_charge_observations import verify_early


class JointZeroChargeObservationTests(unittest.TestCase):
    def fixture(self,cost=0):
        base=load_config(Path(__file__).resolve().parents[1]/'experiments/v0/darwin-baseline.toml')
        world=initialize(base,'block',40,cost,23)
    ...
    def test_birth_charge_and_child_transfer_corruption_are_rejected(self):
        for cost in (0,4):
            initial,metrics,streams=self.fixture(cost)
            result=verify_early(initial,metrics,streams)
            self.assertEqual(sum(c['birth_paid'] for c in result['cohorts'].values()), cost*metrics[-1]['births'])
            self.assertGreater(metrics[-1]['births'],0)
            for corruption in ('birth_paid','child_energy'):
                bad=copy.deepcopy(streams)
                row=next(r for r in bad['energy'] if r['child_id'] is not None)
                row[corruption]+=1
                row['energy_after_action']-=1
                with self.assertRaisesRegex(ValueError,'energy ledger'):
                    verify_early(initial,metrics,bad)
