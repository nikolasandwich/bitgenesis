...
class JointZeroChargeDesignTests(unittest.TestCase):
    def test_all_four_initial_treatments_match_and_observer_preserves_dynamics(self):
        base=load_config(Path(__file__).resolve().parents[1]/'experiments/v0/darwin-baseline.toml')
        founders=rng=None; maps={}
        for arm,threshold,cost in product(('block',),(40,160),(0,4)):
            ...
            self.assertEqual(world.config.birth_cost,cost)
            ...
            for tick in range(10):
                ...
                if cost==0:self.assertTrue(all(r['birth_paid']==0 for r in rows))
