...
变为 7/10，食物组由 1/10 变为 10/10。它尚未加入现有九轮视觉快照和本地归档。
...
本地当前验收入口为 `data/review-v0-9.html`，双击可查看中文研究摘要，并进入世界回放和
谱系检查页。原始实验输出保存在 `data/campaign-001/` 到 `data/campaign-009/`；
这些大文件未推送，精简结果 CSV 和复现脚本已经进入仓库。

视觉入口涵盖九轮、400 个实验、2,040,000 个时间步：
...
重建九轮页面使用 `python scripts/build_v0_review.py --campaigns 9`，需要完整本地数据。
