...
最新[第二十轮](campaign-020.md)比较相同名义平均补给下的三种到达方式：实际供给仍不同，低阈值存活为 3/10、1/10、3/10。下述二十轮整包已包含完整正式数据。

## 下载与打开

下载[二十轮完整验收草稿](https://github.com/nikolasandwich/bitgenesis/releases/tag/untagged-8d064a5f3fad65f69cbf)，需要有权限的 GitHub 账号查看。
完整解压 `bitgenesis-v0-twenty-campaigns.zip` 到新目录，从 `START-HERE.txt` 开始，
再打开 `bitgenesis/data/review-v0-20.html`。保留目录结构，页面依赖相邻图表和世界回放。

整包约 149 MB，含 2,323 个文件，固定源码 `58d4c02`。包括二十轮正式原始数据、
第十八至二十轮前百步个体观测、验收页、演示与代码。它不会随 main 后续提交自动改变。
...
包含原第十九轮数据、全部四十个末期窗口及后来分析，八项解压复算一致。二十轮整包未包含这批后来追加的记录。

第二十轮新增的库存回放另见[库存观测补充包](../design/renewal-stock-supplement.md)，约 40.2 MB，包含原第二十轮与全部六十个百步库存窗口，七份报告解压复算一致。二十轮整包不含这批后来追加的回放。

| 材料 | 适合查看 | 覆盖范围 |
| --- | --- | --- |
| 二十轮整包 | 世界回放、谱系与正式实验验收 | 正式 001–020、早期观测与演示 |
...
- 归档的文件大小和 SHA-256 与 GitHub 返回值一致；13 个 HTML 页和 20 个本地目标通过静态检查。
- 新目录解压后，第二十轮指标、个体观测和过程汇总三份完整报告复算一致。
- 从解压源码新建环境并正式安装，170 项测试通过；千步演示通过记录审计。
- 新演示的指标与事件和归档演示逐字节一致，谱系与摘要 JSON 一致。
- 归档源码通过[CI 34819975721](https://github.com/nikolasandwich/bitgenesis/actions/runs/34819975721)。

这些检查不是全二十轮重新模拟、断网安装保证或浏览器所有交互的重新验收，也不证明生物真实性。
详细记录见[解压复算](results/portable-review-020.json)、[独立安装](results/portable-wheel-020.json)
和[上传核验](results/release-020.json)。整包 SHA-256：
