...
100,000 步，仍是有限时间证据。现有十轮页面和九轮归档尚不包含这次延长复查。
第十轮[繁殖阈值干预](campaign-010.md)发现：提高阈值后，体内储能组终点存活由 0/10
变为 7/10，食物组由 1/10 变为 10/10。它已加入新的十轮页面，但不在九轮归档内。
第九轮的[等能量分配实验](campaign-009.md)发现：相同初始总能量下，环境食物组更易建立，
体内储能组十次全部早期灭绝。草稿归档尚不包含这一轮；新的九轮页面已包含。
...
本地当前验收入口为 `data/review-v0-10.html`，双击可查看中文研究摘要，并进入世界回放和
谱系检查页。原始实验输出保存在 `data/campaign-001/` 到 `data/campaign-009/`；
这些大文件未推送，精简结果 CSV 和复现脚本已经进入仓库。

视觉入口涵盖十轮、440 个实验、2,440,000 个时间步：
...
重建十轮页面使用 `python scripts/build_v0_review.py --campaigns 10`，需要完整本地数据。
