...
从归档源码新建独立安装也通过了 78 项测试；新跑的千步演示事件与指标和原演示逐字节一致。
...
该轮报告和后续运行失败状态修复在 main 中，**不在十四轮归档里**。
...
归档源码 `6d98fb9` 的 78 项测试在 Windows/Linux × Python 3.12/3.13/3.14
六种组合通过（[CI 34786711417](https://github.com/nikolasandwich/bitgenesis/actions/runs/34786711417)）。
main 的已核验源码 `8cbbadf` 则有 89 项测试，同样通过上述六种组合
（[CI 34788020711](https://github.com/nikolasandwich/bitgenesis/actions/runs/34788020711)），
包括后续失败状态、延长复查和执行计数检查。
测试覆盖能量收支、空间约束、遗传、记录核对和精确恢复等契约；通过测试不证明生物学真实性。
冻结重放只对已检查的运行与环境提供证据，不保证所有版本任意运行都逐位一致。

## 研究范围与继续方向

目前十五轮共 **714 次执行、6,340,000 个计算时间步**。其中 24 次是既有世界延长复查，
