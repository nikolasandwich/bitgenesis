...
从提交 `2a8f24c` 用 Git 导出干净源码，仅依赖包内精简指标报告复算，全部六组分段、终点计数、分析范围及输入哈希一致。唯一不同字段是脚本原始字节哈希：本地文件采用 LF，本次 Git 导出为 CRLF；将换行统一后，两份脚本逐字节相同。详见[源码复算记录](results/capacity-horizons-source-review-021.json)。
