...
reviewed here is `58d4c0236fa04ba9b3fa32ef28abcb52cfb6d9d4`; older experiments
...
The archived 170-test suite passed in a fresh installation at this review. This is a V0 milestone assessment,
...
| Reproduce events for fixed rules, seed and configuration | Same test module: frozen 120-tick digest and two-world event/state equality; [portable installation report](../research/results/portable-wheel-020.json) | A fresh installed archive reproduces the saved 1,000-tick demo events and metrics byte for byte. Evidence is for these checked trajectories/environments, not arbitrary future Python versions. |
...
The portable installation evidence concerns fixed archive source `58d4c02` and its
170 tests. The [extraction report](../research/results/portable-review-020.json)
also reproduces all three campaign-020 analysis reports from archived files.
The [local-resource supplement](../design/local-resource-supplement.md), revision 2,
source `3b7ba66`, supplies the retrospective dataset and eight exactly reproduced
reports. That separate dataset is not included in the twenty-campaign archive;
its verification does not claim another fresh runtime installation.
...
| Does equal nominal mean supply yield interchangeable outcomes? | [Campaign 020](../research/campaign-020.md): low-threshold survival 3/10, 1/10, 3/10; rare-large/reference pairs include both directions; all high-threshold worlds survive | Realized supply differs, so this is not a pure-variance intervention or evidence of information use |

Formal totals are twenty campaigns / 964 executions / 8,840,000 computed ticks,
