...
`python scripts/package_v0_review.py --campaigns 14` includes the local raw
outputs for campaigns 001–014 and `data/review-v0-14.html`, along with tracked
source, compact results and the acceptance demonstration. Generate the page first
with `python scripts/build_v0_review.py --campaigns 14`. Both commands require
...
campaigns. For fourteen campaigns it identifies twenty longitudinal follow-ups
and 200000 replayed prefix ticks. Packaging reruns the existing full-run audits,
campaign-005 audit, and selected 009–014 metric verifiers. The 011 verifier also
