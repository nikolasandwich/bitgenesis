"""Independent spatial reconstruction using recorded initial RNG states."""
            ...
            for r in row['actors']:
                ...
                if energy > 0:
                    ...
                    if energy > 0:
                        ...
                        require(bool(r['child_energy']) == expected_birth, 'spatial birth eligibility')
                        if expected_birth:
                            child_position = streams['birth'].choice(free)
                            event = next(tick_events, None)
                            require(event is not None and event['event'] == 'birth'
                                    and event['parent'] == identifier and (event['x'],event['y']) == child_position, 'birth position')
