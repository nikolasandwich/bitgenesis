"""Bounded persisted V1 runs, separate from all V0 execution routes."""
import argparse
...
def run(output, config, seed, steps, mode='intact', max_actor_records=1000000,
    ...
    metadata = {'schema': 'v1-run-1', 'rules': RULES_VERSION,
    ...
    try:
        ...
        summary = {'steps': world.tick, 'population': len(world.organisms),
                   ...
                   'births': len(world.lineage) - config.founders,
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='Experimental BitGenesis V1 runner')
