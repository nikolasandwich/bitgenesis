"""V3 construction/life-history ledger audit, independent of world execution."""
from collections import Counter,defaultdict
from hashlib import sha256
import json
from pathlib import Path

from bitgenesis.v2.construction_audit import verify_attempt,require
...
def audit(directory):
    ...
    require(meta['status']=='complete' and meta['schema']=='v3-run-1'
            and meta['rules']=='v3-world-1' and meta['encoding']=='ecological','unsupported run')
    require([a['id'] for a in final['attempts']]==list(range(len(final['attempts']))),'attempt ID coverage')
    for attempt in final['attempts']:
        require(attempt['encoding']=='ecological','attempt encoding')
        genome=attempt['genome']
        require(set(genome)=={'development','allocation_a'} and type(genome['allocation_a']) is int
                and 0<=genome['allocation_a']<=16,'ecological genome')
        verify_attempt({**attempt,'encoding':'developmental','genome':genome['development']})
    construction={'attempts':len(final['attempts']),
                  'construction_cost':sum(a['construction_cost'] for a in final['attempts']),
                  'failure_loss':sum(a['failure_loss'] for a in final['attempts']),
                  'development_audit_sha256':sha256(Path(__file__).parents[1].joinpath('v2/construction_audit.py').read_bytes()).hexdigest()}
    ...
    def process_attempt(iterator,parent,tick,allocation):
        ...
        if parent is None:
            ...
            old_values=old['development']['genes']+[old['allocation_a']]
            new_values=a['genome']['development']['genes']+[a['genome']['allocation_a']]
            changes=[(i,abs(x-y)) for i,(x,y) in enumerate(zip(old_values,new_values)) if x!=y]
            ...
            for i,delta in changes:
                radius=10 if i in (1,3,5,8) else 1
    ...
    total=(sum(initial['food'])+sum(initial['substrate_b']))+c['founders']*c['initial_energy']-initial_cost
    require(total==(sum(initial['food'])+sum(initial['substrate_b']))+sum(alive.values()),'initial energy')
    actor_count=0
    ticks=0
    with (root/'steps.jsonl').open(encoding='utf-8') as stream:
        for ticks,line in enumerate(stream,1):
            ...
            for r in actors:
                ...
                spent+=sum(r[k] for k in ('basal','decision','movement','birth_cost','development_cost','failure_loss','dissipated'))
    ...
    require(total==sum(final['food'])+sum(final['substrate_b'])+sum(alive.values()),'final total energy')
    ...
    expected={'steps':ticks,'population':len(alive),'individuals':len(born),'actor_records':actor_count,
              ...
              'resource_flows':spatial['resources']['resource_flows'],
              'remaining_a':sum(final['food']),'remaining_b':sum(final['substrate_b'])}
    ...
    return {'scope':'construction, ancestry, decisions, energy and spatial reconstruction; five RNG streams from recorded initial states; initialization/mutation RNG not replayed',
            **{k:v for k,v in spatial.items() if k!='scope'},
