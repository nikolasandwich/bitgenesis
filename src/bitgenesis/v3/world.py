"""Experimental V3 world with inherited two-substrate resource allocation."""
from dataclasses import dataclass, asdict
from hashlib import sha256
from random import Random

from bitgenesis.v1.controller import Genome, inputs, intervene
from bitgenesis.v2.development import develop
from .genome import EcologyGenome
from .resources import feed

RULES_VERSION = "v3-world-1"
...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.initial_b > self.capacity or self.recycling not in (0, 1):
            raise ValueError("invalid B stock or recycling switch")
...
class World:
    def __init__(self, config: Config, seed: int, mode: str = "intact", founder_genomes=None):
        self.encoding = "ecological"
        genetic_type = EcologyGenome
        ...
        self.substrate_b = [config.initial_b] * len(self.food)
        self.organisms = {}
        self.lineage = {}
        self.occupied = {}
        self.events = []
        ...
        expected = sum(self.food) + sum(self.substrate_b) + config.founders * config.initial_energy - self.initialization_spent
    ...
    def _construct(self, parent, x, y, allocation, genome, mode):
        ...
        result = develop(genome.development, allocation)
    ...
    def total_energy(self):
        return sum(self.food) + sum(self.substrate_b) + sum(o.energy for o in self.organisms.values())
    ...
    def step(self):
        ...
        for identifier in actors:
            ...
            record = {"id": identifier, "tick": self.tick, "energy_before": o.energy,
                      ...
                      "intake": 0, "dissipated": 0, "feeding": None, "child_energy": 0, "development_cost": 0, "failure_loss": 0, "action": None, "blocked": False}
            ...
            food = tuple(self.food[y * c.width + x] + self.substrate_b[y * c.width + x]
                         for x, y in [(o.x, o.y)] + neighbors)
            values = inputs(food, 2 * c.capacity, o.energy)
            ...
            a_before, b_before = self.food[site], self.substrate_b[site]
            feeding = feed(a_before, b_before, c.capacity, o.genome.allocation_a,
                           c.feeding_limit, bool(c.recycling))
            self.food[site], self.substrate_b[site] = feeding.remaining_a, feeding.remaining_b
            o.energy += feeding.energy_gain
            record["intake"] = feeding.energy_gain
            record["dissipated"] = feeding.dissipated
            record["feeding"] = dict(site=site, a_before=a_before, b_before=b_before,
                                     allocation_a=o.genome.allocation_a, **asdict(feeding))
        ...
        spent = sum(sum(r[k] for k in ("basal", "decision", "movement", "birth_cost", "development_cost", "failure_loss", "dissipated")) for r in records)
