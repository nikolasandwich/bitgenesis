"""Independent V3 spatial replay, composed with substrate audit."""
...
from .resource_audit import audit as resource_audit
...
def reconstruct(directory):
    resources = resource_audit(directory)
    root = Path(directory)
    def read(name):
    ...
    substrate_b = list(initial['substrate_b'])
    ...
    with (root / 'steps.jsonl').open(encoding='utf-8') as stream:
        for line in stream:
            ...
            for r in row['actors']:
                identifier = r['id']
                check_decision(r, lineage[identifier]['controller']['weights'], lineage[identifier]['mode'])
                position = positions[identifier]
                energy = r['energy_before'] - r['basal'] - r['decision']
                if energy > 0:
                    decisions += 1
                    local = [food[y*width+x] + substrate_b[y*width+x] for x,y in [position] + adjacent(position)]
                    scaled = [1000 * f // (2*c['capacity']) if c['capacity'] else 0 for f in local]
                    ...
                    if energy > 0:
                        ...
                        x,y = position
                        feeding = r['feeding']
                        require(feeding is not None and feeding['site'] == y*width+x, 'feeding position')
                        require(feeding['a_before'] == food[y*width+x]
                                and feeding['b_before'] == substrate_b[y*width+x], 'spatial stocks')
                        intake = feeding['energy_gain']
                        food[y*width+x] = feeding['remaining_a']
                        substrate_b[y*width+x] = feeding['remaining_b']
                        energy += intake
    ...
    require(food == final['food'] and substrate_b == final['substrate_b'], 'final substrate reconstruction')
    ...
    return {'scope': 'resource and spatial replay plus decision arithmetic; recorded lineage/controller and charges still require history audit',
            'resources': resources, 'decisions': decisions, 'spatial_reconstruction': True,
            'decision_audit_sha256': sha256((Path(__file__).parents[1]/'v1/decision_audit.py').read_bytes()).hexdigest(),
