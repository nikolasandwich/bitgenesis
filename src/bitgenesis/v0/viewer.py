...
<p class="note">V0 / v0-darwin-1. Designed organisms and reproduction; this run is not evidence of intelligence or open-ended evolution. Compare multiple seeds and controls. The replay samples frames; metrics cover every tick. Inspect metadata.json, metrics.csv, events.jsonl and lineage.json beside this file for provenance and raw observations.</p>
</main><script>
const data=__PAYLOAD__;const $=id=>document.getElementById(id);const slider=$('scrub');slider.max=data.frames.length-1;
function chart(id,key,color,fixedMax){const el=$(id),ctx=el.getContext('2d'),w=el.width,h=el.height,p=28;const max=fixedMax||Math.max(1,...data.metrics.map(m=>m[key]||0));ctx.clearRect(0,0,w,h);ctx.strokeStyle='#30454c';ctx.beginPath();ctx.moveTo(p,10);ctx.lineTo(p,h-p);ctx.lineTo(w-10,h-p);ctx.stroke();ctx.fillStyle='#a8bab5';ctx.font='11px system-ui';ctx.fillText(String(Math.round(max)),0,15);ctx.fillText('0',8,h-p);ctx.fillText(String(data.metrics.at(-1).tick),w-45,h-8);ctx.strokeStyle=color;ctx.lineWidth=2;ctx.beginPath();let pen=false;data.metrics.forEach((m,i)=>{if(m[key]===null){pen=false;return;}const x=p+i/Math.max(1,data.metrics.length-1)*(w-p-10),y=h-p-m[key]/max*(h-p-10);if(pen)ctx.lineTo(x,y);else ctx.moveTo(x,y);pen=true;});ctx.stroke();}
function draw(){const f=data.frames[Number(slider.value)],m=data.metrics[f.tick],c=data.config,ctx=$('world').getContext('2d'),w=640/c.width,h=640/c.height;ctx.fillStyle='#090f15';ctx.fillRect(0,0,640,640);f.food.forEach((food,p)=>{ctx.fillStyle=`rgba(55,175,112,${food/Math.max(1,c.food_capacity)*0.65})`;ctx.fillRect((p%c.width)*w,Math.floor(p/c.width)*h,w,h);});f.organisms.forEach(([p,g,founder])=>{const hue=$('color').value==='founder'?(founder*137.508)%360:220-g/1000*195;ctx.fillStyle=`hsl(${hue} 75% 65%)`;ctx.fillRect((p%c.width)*w+w*.15,Math.floor(p/c.width)*h+h*.15,w*.7,h*.7);});$('tick').textContent=f.tick;$('population').textContent=m.population;$('generation').textContent=m.max_generation??'—';}
