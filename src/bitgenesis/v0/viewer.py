...
<div class="controls"><button id="play">Play</button><input id="scrub" type="range" min="0" value="0" aria-label="Replay frame" aria-describedby="replayHelp"><select id="color" aria-label="Organism color"><option value="genome">Movement trait</option><option value="founder">Founder lineage</option></select></div>
<div class="legend" id="replayHelp">Use arrow keys to move between saved frames; Home/End jump to the first/last frame. Frames may skip simulation ticks.</div>
...
function draw(){const f=data.frames[Number(slider.value)],m=f.metrics,c=data.config,ctx=$('world').getContext('2d'),w=640/c.width,h=640/c.height;ctx.fillStyle='#090f15';ctx.fillRect(0,0,640,640);f.food.forEach((food,p)=>{ctx.fillStyle=`rgba(55,175,112,${food/Math.max(1,c.food_capacity)*0.65})`;ctx.fillRect((p%c.width)*w,Math.floor(p/c.width)*h,w,h);});f.organisms.forEach(([p,g,founder])=>{const hue=$('color').value==='founder'?(founder*137.508)%360:220-g/1000*195;ctx.fillStyle=`hsl(${hue} 75% 65%)`;ctx.fillRect((p%c.width)*w+w*.15,Math.floor(p/c.width)*h+h*.15,w*.7,h*.7);});$('tick').textContent=f.tick;$('population').textContent=m.population;$('generation').textContent=m.max_generation??'—';slider.setAttribute('aria-valuetext',`Tick ${f.tick}, population ${m.population}, saved frame ${Number(slider.value)+1} of ${data.frames.length}`);}
