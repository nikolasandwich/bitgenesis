...
def audit(directory):
    ...
    require(meta['schema']=='v4-program-assay-1' and meta['rules']=='v4-hereditary-growing-1'
    ...
    require(type(program) is list and len(program)==4 and all(type(x) is int and 0<=x<4 for x in program),'assay program')
    require(type(meta['initial_site']) is int and 0<=meta['initial_site']<meta['width']*meta['height'],'assay initial site')
    ...
    units[meta['initial_site']]=dict(material=meta['initial_material'],energy=meta['initial_energy'],program=program)
    ...
    return dict(scope='independent declared single-founder initialization and drive/direction/mutation RNG, physical dynamics and hereditary expression',
