"""Bounded records of externally driven fixed-site local units."""
import argparse
...
from .driven import step,RULES_VERSION
...
def run(output,seed,steps=100,width=16,height=16,occupancy=750,max_energy=64,
    ...
    values=(seed,steps,width,height,occupancy,max_energy,bond_cost,max_site_records,
            drive_per_thousand,drive_amount,capacity,leak)
    ...
    try:
        ...
        revision=dirty=None
    meta=dict(schema='v4-driven-run-1',rules=RULES_VERSION,status='running',seed=seed,steps=steps,
              ...
              drive_per_thousand=drive_per_thousand,drive_amount=drive_amount,capacity=capacity,leak=leak,
    ...
    try:
        save(root/'initial.json',dict(tick=0,units=snapshot(units),rng_state=rng.getstate(),drive_rng=drive.getstate()))
        ...
        with (root/'steps.jsonl').open('w',encoding='utf-8') as stream:
            for tick in range(1,steps+1):
                ...
                units,record=step(units,width,height,proposals,capacity,leak,bond_cost,exchange)
                energy+=record['imported']-record['spent']
                imported+=record['imported']
                rejected+=record['rejected_import']
                leakage+=record['leakage']
                spent+=record['spent']
                bonds=record['interaction']['bonds']
                stream.write(encode(dict(tick=tick,units=snapshot(units),energy=energy,
                                         components=components(units,bonds),**record)))
        save(root/'final.json',dict(tick=steps,units=snapshot(units),drive_rng=drive.getstate()))
        summary=dict(steps=steps,units=sum(u is not None for u in units),initial_energy=initial_energy,
                     final_energy=energy,spent=spent,imported=imported,rejected_import=rejected,leakage=leakage,last_transition_bonds=len(bonds) if steps else None,
