"""Persist damage/sham continuations of independently verified growing runs."""
...
def run(output, origin, steps, sites=(), threshold=None, max_site_records=1000000):
    ...
    units, raw, boundary = damage(units, before['raw'], sites)
    ...
    try:
        ...
        revision = dirty = None
    meta = dict(schema='v4-damage-branch-1',rules='v4-growing-1',status='running',
    ...
    try:
        ...
        summary = dict(steps=steps,initial_energy=initial_energy,final_energy=energy,
                       ...
                       exported_material=boundary['exported_material'],recycled_material=boundary['recycled_material'])
