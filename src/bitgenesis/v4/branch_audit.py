"""Independent origin, extraction and continued growing dynamics verification."""
...
def audit(directory, origin):
    ...
    require(header['schema']=='v4-branch-1' and header['rules']=='v4-growing-1'
    ...
    raw=list(before['raw'])
    boundary=dict(rules='v4-removal-1',sites=sites,removed=removed,
        exported_material=len(removed),exported_energy=sum(u['energy'] for u in removed),
        material_before=sum(raw)+sum(u is not None for u in before['units']),
    ...
    require(read('boundary.json')==boundary,'boundary extraction mismatch')
    ...
    summary=dict(steps=header['steps'],initial_energy=start,final_energy=energy,
        ...
        units=sum(u is not None for u in units),exported_energy=boundary['exported_energy'],exported_material=boundary['exported_material'])
    require(read('summary.json')==summary,'branch summary mismatch')
    return dict(scope='independent origin reconstruction, extraction, continued RNG and growing dynamics',
