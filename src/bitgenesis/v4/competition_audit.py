...
def audit(directory):
    ...
    require(meta['schema']=='v4-competition-1' and meta['rules']=='v4-hereditary-growing-1'
    ...
    require(meta['mutation_per_thousand']==0,'assay mutation must be disabled')
    programs=meta['programs']
    require(type(programs) is list and len(programs)==2 and all(type(p) is list and len(p)==4 and all(type(x) is int and 0<=x<4 for x in p) for p in programs),'competition programs')
    sites=meta['initial_sites']
    require(type(sites) is list and len(sites)==2 and all(type(i) is int and 0<=i<meta['width']*meta['height'] for i in sites) and len(set(sites))==2,'competition sites')
    require(type(meta['initial_material']) is int and 0<=meta['initial_material']<4,'assay material')
    require(type(meta['initial_energy']) is int and 0<=meta['initial_energy']<=meta['capacity'],'assay initial energy')
    units=[None]*(meta['width']*meta['height'])
    for site,program in zip(sites,programs):
        units[site]=dict(material=meta['initial_material'],energy=meta['initial_energy'],program=program)
    ...
    require(initial==dict(tick=0,units=units,raw=raw,mutation_rng=json.loads(json.dumps(mutation_rng.getstate())),direction_rng=json.loads(json.dumps(direction.getstate())),
    ...
    return dict(scope='independent declared two-founder initialization and drive/direction/mutation RNG, physical dynamics and hereditary expression',
