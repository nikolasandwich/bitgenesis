"""Summarize preregistered early processes from campaign-018 verified reports."""
import argparse
import csv
import hashlib
import json
...
def main():
    ...
    mp=root/'docs/research/results/campaign-018-verification.json'
    op=root/'docs/research/results/campaign-018-observations.json'
    ...
    fields=('arm','birth_threshold','movement_cost','seed')
    index={tuple(r[k] for k in fields):r for r in metrics['runs']}
    records=observations['results'];grid={(a,t,c,s) for a in ('dispersed','block') for t in (40,160) for c in (0,1) for s in range(1500,1510)}
    if len(records)!=80 or {tuple(r[k] for k in fields) for r in records}!=grid or len(metrics['runs'])!=80 or set(index)!=grid:
    ...
    for record in records:
        ...
        if f['eaten']+d['eaten']!=m['food_eaten_by_100'] or f['ending_energy']+d['ending_energy']!=m['organism_energy_at_100'] or f['birth_paid']+d['birth_paid']!=4*m['births_at_100']:
            raise ValueError('Cohort processes disagree with metrics')
        if key['movement_cost']==0 and (f['movement_paid']+d['movement_paid'] or counts['movement_payment_deaths']):
    ...
    for arm in ('dispersed','block'):
        for threshold in (40,160):
            for cost in (0,1):
                subset=[r for r in rows if (r['arm'],r['birth_threshold'],r['movement_cost'])==(arm,threshold,cost)]
                ranges={}
                for k in values:
                    available=[r[k] for r in subset if r[k] is not None]
                    ranges[k]=[min(available),max(available)] if available else None
                groups.append(dict(arm=arm,birth_threshold=threshold,movement_cost=cost,ranges=ranges))
