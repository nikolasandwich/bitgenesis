"""Summarize preregistered early processes from campaign-020 verified reports."""
import argparse
import csv
import hashlib
import json
...
def main():
    ...
    mp=root/'docs/research/results/campaign-020-verification.json'
    op=root/'docs/research/results/campaign-020-observations.json'
    ...
    records=observations['results'];grid={(a,t,c,s) for a in ('block',) for t in (40,160) for c in ("frequent-small","reference","rare-large") for s in range(1700,1710)}
    if len(records)!=60 or {tuple(r[k] for k in fields) for r in records}!=grid or len(metrics['runs'])!=60 or set(index)!=grid:
    ...
    for arm in ('block',):
        for threshold in (40,160):
            for renewal in ("frequent-small","reference","rare-large"):
                subset=[r for r in rows if (r['arm'],r['birth_threshold'],r['renewal'])==(arm,threshold,renewal)]
                ranges={}
                for k in values:
                    available=[r[k] for r in subset if r[k] is not None]
                    ranges[k]=[min(available),max(available)] if available else None
                groups.append(dict(arm=arm,birth_threshold=threshold,renewal=renewal,ranges=ranges))
