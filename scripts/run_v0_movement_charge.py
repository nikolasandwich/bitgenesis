"""Preregistered layout by threshold by movement-charge necessity probe."""
...
def initialize(base, arm, threshold, cost, seed):
    if arm not in ("dispersed","block") or threshold not in (40,160) or type(cost) is not int or cost not in (0,1):
        raise ValueError("Undeclared campaign-018 treatment")
...
def main():
    ...
    protocol=dict(protocol="campaign-018-movement-charge-1",rules_version="v0-darwin-1",
    ...
    protocol["protocol_sha256"]=hashlib.sha256((Path(__file__).resolve().parents[1]/"experiments/v0/campaign-018.md").read_bytes()).hexdigest()
    ...
    results=[]
    try:
        for arm,threshold,cost,seed in product(protocol["arms"],protocol["thresholds"],protocol["movement_costs"],protocol["seeds"]):
            world=initialize(base,arm,threshold,cost,seed)
            prefix=f"{arm}-threshold-{threshold}-move-cost-{cost}-seed-{seed}"
            save(prefix+"-initial.json",dict(arm=arm,birth_threshold=threshold,movement_cost=cost,seed=seed,
            ...
            with ExitStack() as stack:
                ...
                for tick in range(10001):
                    ...
                    if 1<=tick<=100:
                        ...
                        if cost==0 and (any(r['movement_paid'] for r in ledgers) or any(r['phase']=='movement' for r in deaths)):
                    ...
                    world.events.clear()
            results.append(dict(arm=arm,birth_threshold=threshold,movement_cost=cost,seed=seed,**row,**observations,
