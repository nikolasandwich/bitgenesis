...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        for script,reference in checks.items():
            flags=["-S"] if script=="summarize_v0_joint_zero_charge.py" else ["-I","-S"]
            command=[sys.executable,*flags,str(root/"scripts"/script),"--output",str(outputs[script])]
            if script=="verify_v0_local_actions.py":
    ...
    manifest=dict(format="bitgenesis-review-1",git_commit=git("rev-parse","HEAD"),observation_revision=1,observation_campaign="019",
