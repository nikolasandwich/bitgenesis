...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("data/bitgenesis-v0-review-6.zip"))
    ...
    for name in ("campaign-001", "campaign-002", "campaign-003", "campaign-004", "campaign-005", "campaign-006", "mutation-calibration-001", "acceptance-v0"):
        directory = root / "data" / name
        if not directory.is_dir():
            raise ValueError(f"Missing review directory: {name}")
        files.extend(path for path in directory.rglob("*") if path.is_file())
    files.append(root / "data" / "review-v0-6.html")
    ...
    manifest = {"format": "bitgenesis-review-1", "git_commit": git("rev-parse", "HEAD"),
                "scope": "Tracked source plus campaigns 001–006, mutation calibration, acceptance demonstration and Chinese review page. Other local data and environments are excluded.",
                "audits": audited,
