...
def main():
    ...
    if args.campaigns >= 23:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            checkpoint_path = temp/'checkpoints'
            commands = [
                ([sys.executable, '-S', str(root/'scripts/verify_v0_common_environment_metrics.py'), '--input', str(root/'data/campaign-023'), '--output', str(metric_path)], metric_path, 'campaign-023-verification.json'),
                ([sys.executable, '-S', str(root/'scripts/verify_v0_common_environment_histories.py'), '--input', str(root/'data/campaign-023'), '--metrics-verification', str(metric_path), '--output', str(history_path)], history_path, 'campaign-023-histories.json'),
                ([sys.executable, '-I', '-S', str(root/'scripts/summarize_v0_common_environment_checkpoints.py'), '--input', str(root/'data/campaign-023'), '--metrics-verification', str(metric_path), '--histories-verification', str(history_path), '--output', str(checkpoint_path)], checkpoint_path/'summary.json', 'campaign-023-checkpoints.json')]
            for command, output, expected_name in commands:
                subprocess.run(command, check=True, stdout=subprocess.DEVNULL)
                actual = json.loads(output.read_text(encoding='utf-8'))
                expected = json.loads((root/'docs/research/results'/expected_name).read_text(encoding='utf-8'))
