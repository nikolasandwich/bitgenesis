"""Verify full V3 pilot002 coverage and apply its fixed viability rule."""
import argparse
...
def summarize(rows):
    expected=set(product((8,16),(30,60),(1,0),range(87000,87005)))
    seen={}
    for row in rows:
        key=tuple(row[k] for k in ('feeding','renewal','recycling','seed'))
    ...
    cells=[]
    for feeding,renewal,recycling in product((8,16),(30,60),(1,0)):
        group=[seen[(feeding,renewal,recycling,seed)] for seed in range(87000,87005)]
        survivors=[r for r in group if r['population']>0]
        cells.append(dict(feeding=feeding,renewal=renewal,recycling=recycling,
    ...
    selected=next((dict(feeding=f,renewal=r) for f,r in ((8,30),(16,30),(8,60),(16,60))
                   if next(c for c in cells if (c['feeding'],c['renewal'],c['recycling'])==(f,r,1))['usable']),None)
    contrasts=[]
    for recycling in (1,0):
        for seed in range(87000,87005):
            for renewal in (30,60):
                low,high=seen[(8,renewal,recycling,seed)],seen[(16,renewal,recycling,seed)]
                contrasts.append(dict(factor='feeding',fixed=renewal,recycling=recycling,seed=seed,
                    population_difference=high['population']-low['population'],birth_difference=high['births']-low['births']))
            for feeding in (8,16):
                low,high=seen[(feeding,30,recycling,seed)],seen[(feeding,60,recycling,seed)]
                contrasts.append(dict(factor='renewal',fixed=feeding,recycling=recycling,seed=seed,
                    population_difference=high['population']-low['population'],birth_difference=high['births']-low['births']))
    return {'runs':40,'cells':cells,'selected':selected,'paired_contrasts':contrasts,
...
def verify(root):
    ...
    if meta['status']!='complete' or meta['completed_runs']!=40 or meta['planned_runs']!=40:
        raise ValueError('incomplete pilot')
    if meta['protocol_sha256']!=sha256(Path('experiments/v3/pilot-002.md').read_bytes()).hexdigest():
    ...
    paired={}
    for row in rows:
        directory=root/f"feeding-{row['feeding']}-renewal-{row['renewal']}-seed-{row['seed']}-recycling-{row['recycling']}"
        ...
        expected={'width':16,'height':16,'capacity':24,'initial_food':12,'founders':32,
                  ...
                  'feeding_limit':row['feeding'],'birth_threshold':1280,'birth_cost':0,
        ...
        pair=(row['feeding'],row['renewal'],row['seed'])
        if pair in paired:
...
if __name__=='__main__':
    ...
    parser.add_argument('--input',type=Path,default=Path('data/v3-pilot-002'))
