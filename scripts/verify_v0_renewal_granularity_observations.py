"""Verify campaign-020 early observations against metric-verified initial states."""
import argparse
import csv
import hashlib
import json
...
if __package__:
    ...
    from .summarize_v0_renewal_granularity import verify_initial
else:
    from verify_v0_movement_charge_observations import verify_early
    from summarize_v0_renewal_granularity import verify_initial
...
def main():
    ...
    records=verification['runs'];grid={(a,t,c,s) for a in ('block',) for t in (40,160) for c in ("frequent-small","reference","rare-large") for s in range(1700,1710)}
    if len(records)!=60 or {(r['arm'],r['birth_threshold'],r['renewal'],r['seed']) for r in records}!=grid:
    ...
    if verification.get('metric_rows_checked')!=600060 or verification.get('initial_states_checked')!=60:
        raise ValueError('Incomplete metric verification')
    results=[];hashes={}
    for r in records:
        key={k:r[k] for k in ('arm','birth_threshold','renewal','seed')}
        prefix=f"{r['arm']}-threshold-{r['birth_threshold']}-renewal-{r['renewal']}-seed-{r['seed']}"
        ...
        verify_initial(initial,r['arm'],r['birth_threshold'],r['renewal'],r['seed'])
    ...
    report=dict(results=results,input_sha256=hashes,verified_world_ticks=6000,
        ...
        initialization_helper_sha256=hashlib.sha256(Path(__file__).with_name('summarize_v0_renewal_granularity.py').read_bytes()).hexdigest(),
    ...
    print(json.dumps(dict(worlds=len(results),world_ticks=6000,energy_records=sum(r['counts']['energy'] for r in results))))
