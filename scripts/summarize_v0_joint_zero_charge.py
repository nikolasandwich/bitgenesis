"""Independently verify campaign-019 initialization, metrics and observations."""
...
def verify_initial(initial, arm, threshold, cost, seed):
    ...
    if type(cost) is not int or cost not in (0,4) or initial.get("birth_cost")!=cost or initial.get("movement_cost")!=0:
        raise ValueError("Initial direct charges differ")
    ...
    normalized={**initial,"config":{**expected,"birth_threshold":40,"movement_cost":1,"birth_cost":4}}
    return verify_geometry(normalized,arm,seed)



def verify_rows(rows, cost, steps=10000):
    if type(cost) is not int or cost not in (0,4):
        raise ValueError("Undeclared birth charge")
    ...
    for tick, row in enumerate(rows):
        ...
        if tick == 0:
            ...
            if (min(births,deaths,supply,dissipated) < 0 or births+deaths > previous["population"]
                    or supply > 4096 or dissipated != previous["population"]+cost*births
...
def main():
    ...
    expected=dict(protocol="campaign-019-joint-zero-charge-1",rules_version="v0-darwin-1",
    ...
    protocol_path=Path(__file__).resolve().parents[1]/"experiments/v0/campaign-019.md"
    ...
    indexed={(r["arm"],r["birth_threshold"],r["birth_cost"],r["seed"]):r for r in records}
    grid=list(product(expected["arms"],expected["thresholds"],expected["birth_costs"],expected["seeds"]))
    ...
    for arm,threshold,cost,seed in grid:
        ...
        record=dict(arm=arm,birth_threshold=threshold,movement_cost=0,birth_cost=cost,seed=seed,**verify_rows(rows,cost),
    ...
    groups=[]
    for arm,threshold,cost in product(expected["arms"],expected["thresholds"],expected["birth_costs"]):
        ...
        groups.append(dict(arm=arm,birth_threshold=threshold,movement_cost=0,birth_cost=cost,runs=10,
    ...
    pairs=[]
    for arm,cost,seed in product(expected['arms'],expected['birth_costs'],expected['seeds']):
        ...
        pairs.append(dict(arm=arm,movement_cost=0,birth_cost=cost,seed=seed,status=status,
    ...
    report=dict(threshold_pairs=pairs,scope="Initial construction and pairing; metric accounting and predeclared observations. Early observer/energy records require a separate verifier; no independent dynamic replay or unique causal mediation claim.",
        ...
        matched_threshold_cost_map_quartets=10,early_uptake_transitions_checked=4000,runs=list(verified.values()),groups=groups,
