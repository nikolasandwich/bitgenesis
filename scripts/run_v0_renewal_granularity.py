"""Preregistered renewal granularity at equal uncapped nominal supply."""
...
def initialize(base, arm, threshold, renewal, seed):
    if arm != "block" or threshold not in (40,160) or renewal not in REGIMES:
        raise ValueError("Undeclared campaign-020 treatment")
    ...
    config=replace(base,seed=seed,initial_food=0,regrowth_probability=probability,regrowth_amount=amount,
                   mutation_probability=0,birth_threshold=threshold,movement_cost=0,birth_cost=0)
...
def main():
    ...
    protocol=dict(protocol="campaign-020-renewal-granularity-1",rules_version="v0-darwin-1",
        seeds=list(range(1700,1710)),arms=["block"],thresholds=[40,160],renewal_regimes=REGIMES,movement_cost=0,birth_cost=0,steps=10000,
    ...
    protocol["protocol_sha256"]=hashlib.sha256((Path(__file__).resolve().parents[1]/"experiments/v0/campaign-020.md").read_bytes()).hexdigest()
    ...
    results=[]
    try:
        for arm,threshold,renewal,seed in product(protocol["arms"],protocol["thresholds"],REGIMES,protocol["seeds"]):
            world=initialize(base,arm,threshold,renewal,seed)
            prefix=f"{arm}-threshold-{threshold}-renewal-{renewal}-seed-{seed}"
            save(prefix+"-initial.json",dict(arm=arm,birth_threshold=threshold,renewal=renewal,movement_cost=0,birth_cost=0,seed=seed,
            ...
            with ExitStack() as stack:
                ...
                for tick in range(10001):
                    ...
                    world.events.clear()
            results.append(dict(arm=arm,birth_threshold=threshold,renewal=renewal,movement_cost=0,birth_cost=0,seed=seed,**row,**observations,
