"""Plot every preregistered renewal-granularity world with explicit right censoring."""
import argparse
import hashlib
import json
...
def main():
    ...
    parser.add_argument('--input',type=Path,default=Path('docs/research/results/campaign-020-verification.json'))
    ...
    if len(rows)!=60 or set(index)!={(a,t,c,s) for a in ('block',) for t in (40,160) for c in ("frequent-small","reference","rare-large") for s in range(1700,1710)}:
        ...
        for j,renewal in enumerate(("frequent-small","reference","rare-large")):
            ...
            for y,seed in enumerate(range(1700,1710)):
                for threshold,offset in ((40,-.15),(160,.15)):
                    r=index[arm,threshold,renewal,seed]
            ...
            ax.set_title({'frequent-small':'Frequent-small (60/1000 x 1)','reference':'Reference (15/1000 x 4)','rare-large':'Rare-large (5/1000 x 12)'}[renewal],loc='left',fontsize=12)
            if i==0:ax.set_xlabel('Extinction tick / observation horizon (log scale)')
            if j==0:ax.set_ylabel('Matched seed')
    fig.suptitle('Equal nominal supply, different arrival patterns',fontsize=16,y=.976)
    fig.text(.5,.916,'Campaign 020: all 60 worlds. Movement and birth charges are zero; six treatments per matched seed.',ha='center',fontsize=10)
    ...
    fig.text(.5,.075,'Nominal mean is 0.06 per site-tick. Capacity truncation means realized supply need not match.',ha='center',fontsize=9)
    ...
    plt.close(fig)
    targets[2].write_text(json.dumps(dict(figure='campaign-020-renewal-granularity-survival-1',rows=rows,
