"""Factorial allocation, reproduction threshold and direct birth-cost assay under unchanged V0 rules."""
...
def main():
    ...
    protocol = {"protocol": "campaign-012-birth-cost-1", "rules_version": "v0-darwin-1",
                "steps": 10000, "seeds": list(range(1000, 1010)), "treatments": [(f"{allocation}-{threshold}-cost-{cost}", food, energy, threshold, cost)
                    for allocation, food, energy in (("food", 5, 24), ("stored", 0, 88))
                    for threshold in (40, 160) for cost in (0, 4)],
    ...
    results = []
    try:
        for treatment, food, energy, threshold, cost in protocol["treatments"]:
            for seed in protocol["seeds"]:
                world = World(replace(base, seed=seed, initial_food=food, initial_energy=energy,
                                      regrowth_probability=15, mutation_probability=0, birth_threshold=threshold, birth_cost=cost))
                ...
                extinction = None
                observations = {}
                early_peak_population = 0
                early_peak_tick = None
                late_start = None
                with (args.output / f"{treatment}-seed-{seed}.csv").open("w", newline="", encoding="utf-8") as stream:
                    ...
                    for tick in range(protocol["steps"] + 1):
                        ...
                        if tick <= 100 and row["population"] > early_peak_population:
                            early_peak_population = row["population"]
                            early_peak_tick = tick
                        if tick == 9000:
                            late_start = row.copy()
                        if row["population"] == 0 and extinction is None:
                            extinction = tick
                        if tick in (10, 100, 500, 5000):
                            observations[f"population_at_{tick}"] = row["population"]
                            observations[f"births_at_{tick}"] = row["births"]
                        world.events.clear()
                results.append({"treatment": treatment, "initial_food": food, "initial_energy": energy, "birth_threshold": threshold, "birth_cost": cost, "seed": seed, **row, **observations,
                                "extinction_tick": extinction, "right_censored": extinction is None,
                                "late_mean_population": mean(late),
                                "early_peak_population": early_peak_population, "early_peak_tick": early_peak_tick,
                                "late_births": row["births"] - late_start["births"],
                                "late_deaths": row["deaths"] - late_start["deaths"]})
