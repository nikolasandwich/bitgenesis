"""Summarize preregistered early processes from campaign-019 verified reports."""
import argparse
import csv
import hashlib
import json
...
def main():
    ...
    mp=root/'docs/research/results/campaign-019-verification.json'
    op=root/'docs/research/results/campaign-019-observations.json'
    ...
    records=observations['results'];grid={(a,t,c,s) for a in ('block',) for t in (40,160) for c in (0,4) for s in range(1600,1610)}
    if len(records)!=40 or {tuple(r[k] for k in fields) for r in records}!=grid or len(metrics['runs'])!=40 or set(index)!=grid:
    ...
    for record in records:
        ...
        if f['eaten']+d['eaten']!=m['food_eaten_by_100'] or f['ending_energy']+d['ending_energy']!=m['organism_energy_at_100'] or f['birth_paid']+d['birth_paid']!=key['birth_cost']*m['births_at_100']:
    ...
    for arm in ('block',):
        for threshold in (40,160):
            for cost in (0,4):
                subset=[r for r in rows if (r['arm'],r['birth_threshold'],r['birth_cost'])==(arm,threshold,cost)]
                ranges={}
                for k in values:
                    available=[r[k] for r in subset if r[k] is not None]
                    ranges[k]=[min(available),max(available)] if available else None
                groups.append(dict(arm=arm,birth_threshold=threshold,birth_cost=cost,ranges=ranges))
